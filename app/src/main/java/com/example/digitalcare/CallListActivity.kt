...
class CallListActivity : AppCompatActivity() {
    ...
    private val voiceLauncher =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            if (result.resultCode == RESULT_OK) {
                ...
                matches?.firstOrNull()?.let { command -> handleVoiceCommand(command) }
            }
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lvContacts.setOnItemClickListener { _, _, pos, _ ->
            ...
            startActivity(Intent(this, DialingActivity::class.java).putExtra("NUMBER", number))
            finish()
        }
